  ...
  setDarkness() {
    this.mesh = new THREE.Mesh(
      ...
      new THREE.ShaderMaterial({
        uniforms: {
          uTime: { value: 0 },
          uColor: { value: new THREE.Color('#244a6e') },
          uLightness: { value: 2.61 },
          uContrast: { value: 1.63 },
        },
        vertexShader: /* glsl */ `
          uniform float uTime;
          varying vec2 vUv;
...
class Jellfish {
  ...
  setBody() {
    ...
    this.material = new CustomShaderMaterial({
      ...
      vertexShader: `
        ${this.bodyVertPars}
        void main() {
          csm_Position = animate(position + normal * distore(position));
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          //csm_DiffuseColor = vec4(animate(position + normal * distore(position)).xzz, 1.0);
          csm_Bump = animate(position + normal * distore(position));
        }
      `,
      uniforms: {
  ...
  setTentacles() {
    ...
    for (let i = 0; i < this.tentaclesCount; i++) {
      const csmMat = new CustomShaderMaterial({
        ...
        fragmentShader: `
          ...
          void main() {
            csm_DiffuseColor = vec4(distore(vPosition).xzz, 1.0);
            //csm_Bump = distore(vPosition);
          }
        `,
        vertexShader: `
          ${this.tentaclesVertPars}
          ...
          void main() {
            csm_Position = distore(position);
            vPosition = position;
          }
        `,
      });

      const mesh = new THREE.Mesh(geometry, csmMat);
